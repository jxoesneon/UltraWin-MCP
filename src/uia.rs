...
impl UIAutomationProvider for UIAutomationBridge {
    fn get_root_json(&self) -> anyhow::Result<Value> {
        let root = self.get_root().map_err(|e| anyhow::anyhow!("Failed to get root: {}", e))?;
        Ok(self.traverse_tree_native(&root, 0))
    }

    fn get_focused_json(&self) -> anyhow::Result<Value> {
        let element = self.get_focused_element().map_err(|e| anyhow::anyhow!("Failed to get focused: {}", e))?;
        Ok(self.traverse_tree_native(&element, 5))
    }

    fn find_element(&self, query: &str) -> anyhow::Result<Option<RECT>> {
        self.find_element_native(query).map_err(|e| anyhow::anyhow!("Search failed: {}", e))
    }
}
...
impl UIAutomationBridge {
    ...
    fn traverse_tree_native(&self, element: &IUIAutomationElement, depth: u32) -> Value {
        ...
        unsafe {
            ...
            let control_type = element.CachedControlType().unwrap_or(UIA_CONTROLTYPE_ID(0)).0;
    ...
    fn get_children_native(&self, element: &IUIAutomationElement) -> Result<IUIAutomationElementArray> {
        unsafe {
